...
int main( int argc, char *argv[] )  {
  ...
  if ( ( memblk = malloc( memSize ) ) != NULL )  {
	...
	snprintf( outMsg, sizeof(outMsg), "malloc() of %d size failed. errno=%d", memSize,
			  errno );
	handle_error( outMsg );
  }
  return retVal;
...
void winFileWipe(char *fName) {
  ...
  snprintf(outMsg, 2048, "%sunmapped...\r",
		   ptr);
  fprintf(stdout, outMsg);
  fflush(stdout);
  outMsg[strlen(outMsg) - 1] = ' ';
...
void unixFileWipe(char *fName) {
  char         outMsg[2048] = { 0 },
 	          *ptr = NULL;
  long         fileNo   = -1;
  const  int   fileMode = ( O_RDWR | O_EXCL | __O_DIRECT | O_NOFOLLOW | O_SYNC );
  int          fSize    = 0;
